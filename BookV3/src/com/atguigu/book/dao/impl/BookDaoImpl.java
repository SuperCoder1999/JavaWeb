...
public class BookDaoImpl extends BaseDao<Book> implements BookDao {
    ...
    public List<Book> queryForPageItemsByPrice(int begin, int pageSize, int min, int max) {
        String sql =
                "select `id` , `name` , `author` , `price` , `sales` , `stock` , `img_path` imgPath from t_book where `price` between ? and ? order by `price` limit ?, ?";
        List<Book> list = queryForMany(sql,Book.class, min, max, begin, pageSize);
        return list;
    }
}
